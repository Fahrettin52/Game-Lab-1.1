...
public class AnimationWalkTermite : MonoBehaviour {

    private static AnimationWalkTermite instance;

    public static AnimationWalkTermite Instance {
        get{
            if (instance == null) {
                instance = FindObjectOfType<AnimationWalkTermite>();
            }
            return instance;
        }
    }
	...
	public void DropDead (int damage){
        ...
        if (livesEnemy == 0 && mayDie == true && mayDrop == true){
            ...
            animator.SetBool("TermSolWalk", false);
            animator.SetBool("TermSolWalkStop", true);
            transform.Find("Termiet soldier model").GetComponent<DamageWithWalk>().damageForPlayer = 0;
            moveSpeed = 0;
            navSpeed = 0;
